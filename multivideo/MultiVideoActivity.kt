...
class MultiVideoActivity : BaseSimpleActivity() {
    ...
    override fun getAppIconIDs() = intArrayOf()
    ...
    private fun setupControls() {
        ...
        // Sincronizar — quando ativo, todos os cliques individuais afetam todos
        binding.multiVideoSync.setOnClickListener {
            isSyncMode = !isSyncMode
            binding.multiVideoSync.setColorFilter(
                if (isSyncMode) 0xFFFFFFFF.toInt() else 0xFF888888.toInt()
            )
    ...
    private fun toggleSingle(index: Int) {
        if (isSyncMode) {
            // Modo sync: play/pause afeta todos
            isAllPlaying = !isAllPlaying
            players.forEachIndexed { i, mp ->
                if (mp != null && isReady[i]) {
                    if (isAllPlaying) mp.start() else mp.pause()
