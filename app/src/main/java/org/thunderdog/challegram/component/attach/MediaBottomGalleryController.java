...
public class MediaBottomGalleryController extends MediaBottomBaseController<MediaBottomGalleryController.Arguments> implements Media.GalleryCallback, MediaGalleryAdapter.Callback, Menu, View.OnClickListener, MediaBottomGalleryBucketAdapter.Callback, MediaViewDelegate, MediaSelectDelegate, MediaSendDelegate {
  ...
  private void selectAllInCurrentBucket () {
    if (currentBucket == null || adapter == null) return;
    ArrayList<ImageFile> images = currentBucket.getMedia();
    if (images == null) return;
    ArrayList<ImageFile> alreadySelected = adapter.getSelectedPhotosAndVideosAsList(false);
    for (ImageFile imageFile : images) {
      boolean alreadyChecked = alreadySelected != null && alreadySelected.contains(imageFile);
      if (!alreadyChecked) {
        adapter.setSelected(imageFile, true);
      }
    }
    mediaLayout.setCounter(adapter.getSelectedCount());
  }
