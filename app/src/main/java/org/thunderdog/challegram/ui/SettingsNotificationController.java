...
public class SettingsNotificationController extends RecyclerViewController<SettingsNotificationController.Args> implements
  ...
  private static final String KEY_DISMISS_FIS_AUTH_ERROR = "tgx_dismiss_fis_auth_error";

  private boolean isDismissedFisAuthError () {
    return "FIS_AUTH_ERROR".equals(tdlib.context().getTokenError()) && Settings.instance().getBoolean(KEY_DISMISS_FIS_AUTH_ERROR, false);
  }
  ...
  private boolean needErrorMode () {
    if (Build.VERSION.SDK_INT < Build.VERSION_CODES.KITKAT || customChatId != 0 || scope != null || !tdlib.notifications().areNotificationsBlockedGlobally()) {
      return false;
    }
    return !isDismissedFisAuthError();
  }
      ...
      switch (status) {
        ...
        case TdlibNotificationManager.Status.PUSH_SERVICE_ERROR: {
          showOptions(new Options.Builder()
            ...
            .item(new OptionItem(R.id.btn_cancel, Lang.getString(R.string.FirebaseErrorResolveDismiss), OptionColor.NORMAL, R.drawable.baseline_close_24))
            .build(), (optionView, optionId) -> {
            if (optionId == R.id.btn_retry) {
              Settings.instance().putBoolean(KEY_DISMISS_FIS_AUTH_ERROR, false);
              tdlib.context().checkDeviceToken(0, null);
            } else if (optionId == R.id.btn_share) {
              shareTokenError();
            } else if (optionId == R.id.btn_cancel && "FIS_AUTH_ERROR".equals(tdlib.context().getTokenError())) {
              Settings.instance().putBoolean(KEY_DISMISS_FIS_AUTH_ERROR, true);
              checkInErrorMode();
            }
            return true;
          });
  ...
  public void onTokenStateChanged (int newState, @Nullable String error, @Nullable Throwable fullError) {
    if (newState == TdlibManager.TokenState.OK) {
      Settings.instance().putBoolean(KEY_DISMISS_FIS_AUTH_ERROR, false);
    }
    runOnUiThreadOptional(this::checkInErrorMode);
  }
