    ...
    fun refreshMenuItems() {
        ...
        runOnUiThread {
            ...
            binding.mediumViewerToolbar.menu.apply {
                ...
                findItem(R.id.menu_ocr).isVisible = currentMedium.isImage() || currentMedium.isVideo()
    ...
    private fun setupOptionsMenu() {
        ...
        binding.mediumViewerToolbar.setOnMenuItemClickListener { menuItem ->
            ...
            when (menuItem.itemId) {
                ...
                R.id.menu_ocr -> {
                    val medium = getCurrentMedium() ?: return@onOptionsItemSelected true
                    val ctx = this
                    Thread {
                        try {
                            val bmp = if (medium.isVideo()) {
                                val retriever = android.media.MediaMetadataRetriever()
                                retriever.setDataSource(medium.path)
                                val pos = (getCurrentFragment() as? com.goodwy.gallery.fragments.VideoFragment)?.getCurrentPosition() ?: 0L
                                val frame = retriever.getFrameAtTime(pos * 1000, android.media.MediaMetadataRetriever.OPTION_CLOSEST)
                                retriever.release()
                                frame
                            } else {
                                ...
                                .addOnSuccessListener { result ->
                                    ...
                                    runOnUiThread {
                                        if (text.isEmpty()) {
                                            ...
                                            toast(R.string.ocr_copied)
                                        }
                                    }
                                    recognizer.close()
                                }
                                .addOnFailureListener { recognizer.close() }
                        } catch (_: Exception) {}
                    }.start()
                }
